...
import { useDashboardStore, type Dashboard, type DashboardUser } from '@/store/dashboardStore';
...
const DashboardUserManagement: React.FC<DashboardUserManagementProps> = ({
  ...
  const { addDashboardUser, removeDashboardUser, updateDashboardUser } = useDashboardStore();
  ...
  const handleAddUser = () => {
    ...
    if (existingUser) {
      toast({
        ...
        description: "A user with this email already has access to this dashboard.",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleRemoveUser = (userId: string) => {
    removeDashboardUser(dashboard.id, userId);
    toast({
      title: "User Removed",
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Select value={newUser.role} onValueChange={(value: 'viewer' | 'editor') => setNewUser(prev => ({ ...prev, role: value }))}>
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <CardContent>
                  {dashboardUsers.length === 0 ? (
                    <div className="text-center py-8 text-muted-foreground">
                      ...
                      <p className="text-sm">Add users above to grant them access.</p>
                    </div>
                  ) : (
